...
function formatPrefix(method, url) {
	...
	return {
		...
		hooks: {
			...
			async onSuccess(context) {
				...
				log(`${formatPrefix(context.request.method, context.request.url)} — ${status} ${statusText}${duration}`);
				if (opts.verbose) opts.console.log(context.data);
			},
			onRetry(response) {
				if (!enabled) return;
				const log = opts.console.warn || opts.console.log;
				if (isLegacy) {
					log("Retrying request...", "Attempt:", (response.request.retryAttempt || 0) + 1);
					return;
				}
				const attempt = (response.request.retryAttempt || 0) + 1;
				log(`${formatPrefix(response.request.method, response.request.url)} — Retry attempt #${attempt}`);
			},
			async onError(context) {
				...
				if (isLegacy) {
					...
					log("Request failed with status: ", context.response.status, `(${context.response.statusText || getStatusText(context.response.status)})`);
